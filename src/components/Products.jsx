...
function Products() {
  ...
  useEffect(() => {
    ...
    applyLimit();
    if (mq.addEventListener) mq.addEventListener('change', applyLimit);
    else mq.addListener(applyLimit);

    return () => {
      if (mq.removeEventListener) mq.removeEventListener('change', applyLimit);
      else mq.removeListener(applyLimit);
    };
  }, []);

  const handleShowMore = () => {
    setVisibleCount(Math.min(visibleCount + initialLimit, galleryItems.length));
  };
